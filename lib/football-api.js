...
class FootballAPI {
  ...
  async getTodayMatches(channelLeagues = []) {
    ...
    if (this.fdKey) {
      try {
        // football-data.org requires dateTo > dateFrom to include full day
        const data = await this.retryCall(() =>
          this.fdCall('/matches', { dateFrom: today, dateTo: tomorrow })
        );
        // Filter to only today's matches (tomorrow fetched just to work around API date quirk)
        const todayMatches = (data.matches || []).filter(m => m.utcDate?.startsWith(today));
        fixtures = todayMatches.map(m => this.normalizeFdMatch(m));
  ...
  async getYesterdayResults(channelLeagues = []) {
    ...
    if (this.fdKey) {
      try {
        const data = await this.retryCall(() =>
          this.fdCall('/matches', { dateFrom: yesterday, dateTo: today, status: 'FINISHED' })
        );
        const yesterdayMatches = (data.matches || []).filter(m => m.utcDate?.startsWith(yesterday));
        fixtures = yesterdayMatches.map(m => this.normalizeFdMatch(m));
