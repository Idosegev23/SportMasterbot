...
Write a VERY SHORT English tip (max 4 lines) that is practical and promotional.
Include:
1) Simple bankroll control
2) Auto Cash Out target (1.2x–1.5x)
3) Optional 2-bet hedge idea
4) Close with a clean CTA using code ${promoCode}

Rules:
- ENGLISH ONLY
- Telegram HTML only (<b>, <i> allowed). No markdown
- No promises of profit, no guarantees
- No emojis
`;
      ...
      // Lightweight header
      if (!/^Aviator/i.test(content)) {
        content = `<b>Aviator Quick Tip</b>\n\n` + content;
      }
      return content;
    } catch (e) {
      console.error('Error generating Aviator quick tip:', e);
      return `<b>Aviator Quick Tip</b>\n\nUse Auto Cash Out around 1.3x, keep stakes small, and consider hedging with two bets. Stay disciplined and keep sessions short.\n\nUse code ${promoCode}`;
    }
  }
...
Create a SHORT English session plan with:
- <b>Title</b>
- 3 bullet lines:
  1) Stake range (e.g., 0.1–0.3)
  2) Auto Cash Out target (e.g., 1.35x)
  3) Simple win target / session length guidance
- End with a clean CTA mentioning code ${promoCode}

Rules:
- ENGLISH ONLY, Telegram HTML (<b>, <i>)
- Max ~5 lines total
- No emojis, no guarantees, no links
`;
      ...
      let content = this.cleanTelegramHTML(response.choices[0].message.content.trim());
      if (!/^<b>.*Aviator/i.test(content)) {
        content = `<b>Aviator Session Plan</b>\n` + content;
      }
      return content;
    } catch (e) {
      console.error('Error generating Aviator session plan:', e);
      return `<b>Aviator Session Plan</b>\n- Stakes: 0.1–0.3\n- Auto Cash Out: ~1.35x\n- Short sessions, clear goal.\nUse code ${promoCode}`;
    }
  }
...
Write 2–3 SHORT sentences promoting Aviator (crash) sessions. Mention code ${promoCode}. No hashtags, no links, no emojis. Telegram HTML only.`;
      ...
      return `<b>Aviator Promo</b>\n\nJump in with tight targets and fast cash outs. Keep it sharp and controlled.\n\nUse code ${promoCode}`;
    }
  }
