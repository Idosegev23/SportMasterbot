...
class TelegramManager {
  ...
  // Helper function to generate images with timeout
  async generateImageWithTimeout(imagePromise, description = 'image') {
    const TIMEBOX_MS = Number(process.env.AI_IMAGE_TIMEOUT_MS || 180000); // 3 minutes
    ...
    try {
      ...
      if (!imageBuffer) console.log(`⏱️ ${description} generation skipped (timeboxed after 3 minutes)`);
      return imageBuffer;
    } catch (e) {
  ...
  async sendPredictions(predictions, matches = null) {
    ...
    // Extract match metadata for tracking
    ...
    try {
      ...
      if (!imagesDisabled && matches && matches.length > 0) {
        ...
        if (String(process.env.AI_IMAGE_NO_TIMEOUT || '').toLowerCase() === 'true') {
          ...
          // Extended timeout for gpt-image-1 which needs more time
      ...
      const keyboard = await this.createPredictionsKeyboard();
        ...
        reply_markup: {
          inline_keyboard: keyboard
              ...
              const metadata = { ...getMatchMetadata(i), matchNumber: i + 1, hasImage: true };
              this.trackMessage('predictions', message.message_id, metadata);
              await this.logPostToSupabase('predictions', formattedContent, message.message_id, metadata);
              ...
              const metadata = { ...getMatchMetadata(i), matchNumber: i + 1, hasImage: false };
              this.trackMessage('predictions', message.message_id, metadata);
              await this.logPostToSupabase('predictions', formattedContent, message.message_id, metadata);
              messageIds.push(message.message_id);
            }
            ...
            const metadata = { ...getMatchMetadata(i), matchNumber: i + 1 };
            this.trackMessage('predictions', message.message_id, metadata);
            await this.logPostToSupabase('predictions', formattedContent, message.message_id, metadata);
            messageIds.push(message.message_id);
          }
  ...
  async sendLivePredictions(predictions, liveMatches = null) {
    ...
    try {
      ...
      if (liveMatches && liveMatches.length > 0) {
        ...
        imageBuffer = await this.generateImageWithTimeout(
          this.imageGenerator.generateLiveImage(liveMatches),
          'live image'
        );
      }
  ...
  async sendResults(content, results = null) {
    return await this.retryRequest(async () => {
      ...
      if (results && results.length > 0) {
        ...
        imageBuffer = await this.generateImageWithTimeout(
          this.imageGenerator.generateResultsImage(results),
          'results image'
        );
      }
  ...
  async sendNews(headlines, originalNewsItem = null) {
    return await this.retryRequest(async () => {
      ...
      try {
        const imagePromise = this.imageGenerator.generateNewsImage(originalNewsItem);
        imageBuffer = await Promise.race([
          imagePromise,
          new Promise((_, reject) => 
      ...
      if (Array.isArray(headlines)) {
        // Legacy format - multiple headlines
        newsText = `📰 *SportMaster Daily News*\n\n${headlines.map((h, i) => `${i + 1}. ${h}`).join('\n\n')}\n\n⏰ ${new Date().toLocaleString('en-US', { timeZone: 'Africa/Addis_Ababa' })} (ET)`;
      } else {
        // New format - single detailed article
        let article = headlines;
        
        // Telegram caption limit is 1024 characters
        const timestamp = `\n\n⏰ ${new Date().toLocaleString('en-US', { timeZone: 'Africa/Addis_Ababa' })} (ET)`;
        const maxLength = 1024 - timestamp.length - 50; // Leave some buffer
        
        if (article.length > maxLength) {
          // Truncate and add ellipsis
          article = article.substring(0, maxLength - 3) + '...';
          console.log(`📝 News truncated to ${article.length} characters for Telegram limits`);
        }
  ...
  // Send Aviator content with optional AI image
  async sendAviator(content, variant = 'session', promoCode = 'SM100') {
    return await this.retryRequest(async () => {
      // Reuse promo-style keyboard (coupon + personal)
      const keyboard = await this.createPromoKeyboard(promoCode);
      let imageBuffer = null;
      try {
        imageBuffer = await this.imageGenerator.generateAviatorImage(variant, promoCode);
      } catch (_) {}

      const formatted = content; // already HTML
      if (imageBuffer) {
        try {
          const message = await this.bot.sendPhoto(this.channelId, imageBuffer, {
            caption: formatted,
          ...
          this.trackMessage(`aviator-${variant}`, message.message_id, { hasImage: true });
          await this.logPostToSupabase('promo', formatted, message.message_id);
          return message;
        } catch (_) { /* fallback to text */ }
      }

      const msg = await this.bot.sendMessage(this.channelId, formatted, {
        parse_mode: 'HTML',
        disable_web_page_preview: true,
        reply_markup: { inline_keyboard: keyboard }
      });
      this.trackMessage(`aviator-${variant}`, msg.message_id, { hasImage: false });
      await this.logPostToSupabase('promo', formatted, msg.message_id);
      return msg;
    });
  }
  ...
  async logPostToSupabase(contentType, content, messageId, metadata = {}) {
    try {
      if (!supabase) return;
      
      // Save to posts table for general tracking
      const typeMap = {
      ...
      // Save to telegram_posts for detailed tracking (especially for predictions)
      await supabase.from('telegram_posts').insert({
        message_id: messageId,
        type: contentType,
        content: content.substring(0, 1000), // Truncate content for storage
        metadata,
        created_at: new Date().toISOString()
      });

      console.log(`✅ Logged ${contentType} to Supabase with metadata:`, metadata);
    } catch (e) {
