...
class FootballAPI {
  ...
  filterPopularTeamMatches(matches) {
    ...
    const fallbackFiltered = matches.filter(match => {
      ...
      // Exclude youth teams, reserves, and irrelevant leagues
      const isYouthOrReserve = leagueName.includes('u21') || 
                              leagueName.includes('u20') || 
                              leagueName.includes('u19') || 
                              leagueName.includes('youth') || 
                              leagueName.includes('reserve') || 
                              leagueName.includes('academy') ||
                              homeTeam.includes('u21') || 
                              awayTeam.includes('u21') ||
                              homeTeam.includes('u20') || 
                              awayTeam.includes('u20');
      
      // Exclude very obscure leagues or countries
      const isObscureLeague = leagueName.includes('kecskeméti') ||
                             leagueName.includes('fehérvár') ||
                             leagueName.includes('volta redonda') ||
                             leagueName.includes('crb') ||
                             leagueName.includes('transport united') ||
                             leagueName.includes('tensung') ||
                             leagueName.includes('kudrivka') ||
                             leagueName.includes('sk poltava');
      
      if (isYouthOrReserve || isObscureLeague) {
        return false;
      }
  ...
  async getYesterdayResults() {
    try {
      ...
      // Limit to top 5 most important results to avoid spam
      // Sort raw results first by match score, then process them
      const sortedResults = popularResults
        .sort((a, b) => this.calculateMatchScore(b) - this.calculateMatchScore(a))
        .slice(0, 5);
      
      const topResults = this.processResults(sortedResults);
      
      console.log(`📊 Limiting to top ${topResults.length} results for cleaner presentation`);
      return topResults;
    } catch (error) {
    ...
    try {
      ...
      const response = await this.retryApiCall(async () => {
        return await axios.get(`${this.baseUrl}/fixtures`, {
          ...
          params: {
            date: today
            // Remove status filter to include both upcoming and live matches
          },
          timeout: 30000
        });
      });
      ...
      // Filter to leagues with meaningful scores first to avoid low-quality matches
      const qualityMatches = allFixtures.filter(fixture => {
        const leagueScore = this.getLeagueScore(fixture.league.id);
        return leagueScore >= 50; // Only leagues with score 50+ (excludes low-tier leagues)
      });
      
      console.log(`🔍 Filtered to ${qualityMatches.length} matches from quality leagues`);
      
      // Score and rank quality matches only
      const scoredMatches = qualityMatches
        .map(fixture => ({
          ...fixture,
  ...
  // Get league score by ID for filtering
  getLeagueScore(leagueId) {
    const leagueScoresById = {
      ...
      3: 120,
      // Portuguese Primeira Liga (ID: 94)
      94: 90,
      // Dutch Eredivisie (ID: 88)
      88: 85,
      // English Championship (ID: 40)
      40: 80,
      // Turkish Süper Lig (ID: 203)
      203: 75,
      // Italian Serie B (ID: 136)
      136: 70,
    ...
    return leagueScoresById[leagueId] || 15; // Default for unknown leagues
  }
  ...
  calculateMatchScore(match) {
    ...
    // League importance - use both ID and name for accuracy
    const leagueScoresById = {
      // English Premier League (ID: 39)
      39: 130,
      // Spanish La Liga (ID: 140)  
    ...
    // Fallback to name-based scoring for leagues without ID mapping
    const leagueScoresByName = {
      'UEFA Champions League': 150,
      'UEFA Europa League': 120,
      'Brasileirão Serie A': 90,
      'MLS': 65,
      'Saudi Pro League': 60
    };
    
    // Use ONLY ID-based scoring (more accurate) - ignore name-based to avoid confusion
    let baseLeagueScore = leagueScoresById[match.league.id];
    if (!baseLeagueScore) {
      // If league ID not in our popular leagues, give very low score regardless of name
      // This prevents Egyptian "Premier League" from getting high scores
      baseLeagueScore = 15; // Much lower default for unknown leagues
    }
    score += baseLeagueScore;
    
    // Additional bonus for top-tier leagues to ensure they dominate (ID-based)
    const topTierLeagueIds = [2, 3, 39, 140, 135, 78, 61]; // Champions, Europa, Premier, La Liga, Serie A, Bundesliga, Ligue 1
    if (topTierLeagueIds.includes(match.league.id)) {
      score += 50; // Extra bonus for top leagues
    }
  ...
  processMatches(matches) {
    return matches
      // Include all matches (NS, TBD, 1H, 2H, HT, FT) - no status filtering
      .map(match => ({
        id: match.fixture.id,
        homeTeam: {
  ...
  async getLiveMatches() {
    ...
    try {
      ...
      // Process live matches with enhanced info
      const liveMatches = await Promise.all(filteredFixtures.map(async fixture => {
        const baseMatch = {
        ...
        // Get match events for more detailed info
        try {
          const eventsResponse = await axios.get(`${this.baseUrl}/fixtures/events`, {
            headers,
            params: {
          ...
          // Process recent events (goals, cards, substitutions)
          const recentEvents = events
            .filter(event => ['Goal', 'Card', 'subst'].includes(event.type))
            .slice(-3) // Last 3 events
            .map(event => ({
              type: event.type === 'subst' ? 'Substitution' : event.type,
              minute: event.time.elapsed,
              player: event.player?.name || event.assist?.name,
              team: event.team?.name
            }));

          baseMatch.events = recentEvents;
        } catch (eventError) {
          console.log(`⚠️ Could not fetch events for ${fixture.fixture.id}:`, eventError.message);
          baseMatch.events = [];
        }
