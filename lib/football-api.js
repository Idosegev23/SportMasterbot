...
class FootballAPI {
  ...
  calculateMatchScore(match) {
    ...
    const baseLeagueScore = leagueScores[match.league.name] || 30; // Lower default for unknown leagues
    score += baseLeagueScore;
    
    // Additional bonus for top-tier leagues to ensure they dominate
    const topTierLeagues = ['UEFA Champions League', 'UEFA Europa League', 'Premier League', 'La Liga', 'Serie A', 'Bundesliga', 'Ligue 1'];
    if (topTierLeagues.includes(match.league.name)) {
