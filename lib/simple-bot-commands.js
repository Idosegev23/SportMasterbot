...
👤 <b>Admin:</b> ${new Date().toLocaleString('en-US', { timeZone: 'Africa/Addis_Ababa' })} (ET)`;
  ...
  async handleMyLink(msg) {
    ...
    const portalUrl = `${baseUrl}/portal/${channel.owner_token}`;
...
const keyboard = [[{ text: '👤 Get Personal Coupons', url: 'https://t.me/Sportmsterbot?start=join_personal' }]];
  ...
  async executeNews(chatId) {
    try {
      await this.bot.sendMessage(chatId, '📰 Fetching latest sports news from RSS feeds...');
      ...
      if (typeof result === 'string') {
        await telegram.sendNews(result);
      } else {
        await telegram.sendNews(result.content, result.originalItem);
      }
