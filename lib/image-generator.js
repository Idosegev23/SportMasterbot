...
class ImageGenerator {
  ...
  async retryImageGeneration(generateFn, maxRetries = 2, baseDelay = 5000) {
  ...
  async generateTodayHypeImage(matches) {
    try {
      ...
      return await this.retryImageGeneration(async () => {
...
`;
        ...
        console.log('🎯 Generating today hype image with AI... (this may take up to 2 minutes)');
        ...
        console.log(`⏱️ Today hype image generation completed in ${generationTime}s`);

        const imageBase64 = response.data[0].b64_json;
        console.log('✅ Today hype image generated successfully');
      ...
      if (err.code === 'ETIMEDOUT' || err.message?.includes('timeout')) {
        console.error('⏰ Timeout error: Today hype image generation took too long. Try reducing prompt complexity.');
      } else if (err.status === 429) {
        console.error('🚫 Rate limit exceeded. Please wait before trying again.');
      } else if (err.status === 400) {
        console.error('❌ Bad request: Check prompt parameters and model compatibility.');
      }
...
- Composition: square (1:1).`;
        ...
        console.log('🎨 Generating prediction image with AI... (this may take up to 2 minutes)');
        const startTime = Date.now();
        ...
        const generationTime = ((Date.now() - startTime) / 1000).toFixed(1);
        console.log(`⏱️ Image generation completed in ${generationTime}s`);
      ...
      if (error.code === 'ETIMEDOUT' || error.message?.includes('timeout')) {
        console.error('⏰ Timeout error: Image generation took too long. Try reducing prompt complexity.');
...
`;
  ...
  // Generate news image with dynamic content
  async generateNewsImage(newsItem = null) {
    try {
      // Prepare news content with fallbacks
      const headline = newsItem?.title || 'Sports News Update';
      const source = newsItem?.source || 'SportMaster';
      const date = newsItem?.pubDate || 'Recent';
      const description = newsItem?.description || 'Latest sports developments';

      const prompt = `Create a vibrant sports news image for a premium Telegram bot with a cutting-edge, high-tech aesthetic, dynamically featuring article content.
Article Content

Headline: "${headline}"
Key Details: "${description}" (e.g., teams, dates).
Source: ${source}
Date: ${date}

Background

Smooth gradient from deep navy (#1a237e) to electric blue (#1976d2) with a faint geometric grid.
Subtle glowing ticker lines and a floating soccer ball icon for a live broadcast vibe.

Foreground

Sleek holographic news desk with a large screen displaying "${description}" (e.g., team logos or stats).
Bold "BREAKING NEWS" banner in red with electric blue glow, featuring "${headline}" in sharp white text.
Floating sports icons (soccer ball, basketball) with light particle effects.

Key Elements

Prominent "📰 SPORTS NEWS" header in metallic silver with a neon blue outline.
Dynamic news ticker at the bottom showing "${description}" in concise, scrolling text.
"LIVE UPDATE" badge with a pulsing red dot in the top corner.
Clock icon with neon blue glow displaying current time (e.g., 03:07 PM IDT, Aug 14, 2025).

Branding

"SportMaster" logo in bold, premium font with a subtle silver shadow, placed bottom-left.
Tagline "Your Premier Sports Update Source" in sleek, italicized typography below the logo.

Colors & Typography

Palette: Navy blue, electric blue, white, silver, with red for urgency.
High-contrast, modern fonts (e.g., sans-serif) for readability and a polished look.

Visual Style

Futuristic news studio with dynamic lighting, clean lines, and a professional yet energetic feel.
Emphasize a fan-friendly, live-broadcast atmosphere with bold, uncluttered design.

Critical Requirements

Max 300 characters for description.
Ensure "${headline}" and "${description}" are clearly visible on the banner and screen/ticker.
Avoid clutter; prioritize branding and article content.
Match a lively, passionate sports news tone.`;
  ...
  // Generate Aviator (crash) image variants
  async generateAviatorImage(variant = 'session', promoCode = 'SM100') {
    try {
      const basePrompt = `Create a square 1:1 premium Aviator (crash) poster. Show a rising multiplier curve (x1.00→x1.50+), a sleek plane icon, and a glowing 'Cash Out' button UI. 
Futuristic, high-contrast, dark background with neon red/blue accents. Minimal text. Footer: 't.me/Sportmsterbot' and promo code '${promoCode}'.`;

      const variantLine = variant === 'promo'
        ? 'Highlight a bold "Aviator" title and a subtle promo vibe.'
        : variant === 'tip'
          ? 'Focus on a clean graph and interface cue, less text.'
          : 'Balanced: session vibe with neat cockpit HUD elements.';
