...
⚙️ <b>System</b> — Status & analytics`;
  ...
  // ⚽ Execute predictions via API — channel-aware
  async executePredictions(chatId, messageId) {
    try {
      ...
      const editProgress = async (label) => {
        if (!messageId || Date.now() - lastEdit < 5000) return;
        lastEdit = Date.now();
        try {
          await this.bot.editMessageText(`🤖 <i>Processing...</i>\n\n${label}\n⏳ <b>${Math.floor((Date.now() - startTs)/1000)}s</b>`, { chat_id: chatId, message_id: messageId, parse_mode: 'HTML' });
        } catch (_) {}
      };

      await editProgress('⚽ Preparing predictions...');

      const body = channelConfig ? { channelId: channelConfig.channel_id } : {};
      let response = await axios.post(`${baseUrl}/api/manual/predictions`, body, {
      ...
      if (!response.data.success && /No matches/i.test(response.data.message || '')) {
        ...
        response = await axios.post(`${baseUrl}/api/manual/predictions?source=all`, body, {
          headers: { 'Content-Type': 'application/json', 'x-bot-internal': 'true' },
          timeout: 60000
        });
      }
      ...
      if (response.data.success) {
        ...
        const targetCh = channelConfig?.channel_id || process.env.CHANNEL_ID || '@africansportdata';
        await this.bot.sendMessage(chatId,
          `✅ <b>Predictions sent!</b>\n\n` +
          `📊 ${response.data.message || 'Completed'}\n` +
          `📡 <b>Channel:</b> ${targetCh}\n` +
          `⏰ ${new Date().toLocaleString('en-US', { timeZone: channelConfig?.timezone || 'Africa/Addis_Ababa' })}`,
          { parse_mode: 'HTML' }
        );
      } else {
        await this.bot.sendMessage(chatId, '❌ ' + (response.data.message || 'Unknown error'));
      }
    } catch (error) {
  ...
  // 📋 Execute summary via API — channel-aware
  async executeSummary(chatId) {
    try {
      ...
      const response = await axios.post(`${this.getBaseUrl()}/api/manual/summary`, body, {
        headers: { 'Content-Type': 'application/json', 'x-bot-internal': 'true' },
        timeout: 60000
      });

      const targetCh = channelConfig?.channel_id || process.env.CHANNEL_ID;
      if (response.data.success) {
        await this.bot.sendMessage(chatId, `✅ Summary sent to <b>${targetCh}</b>!`, { parse_mode: 'HTML' });
      } else {
        await this.bot.sendMessage(chatId, '❌ ' + (response.data.message || 'Unknown error'));
      }
    } catch (error) {
  ...
  // 🎁 Execute promo via API — channel-aware
  async executePromo(chatId) {
    try {
      ...
      this._pendingPromo.set(chatId, { baseUrl, channelId: channelConfig?.channel_id });
    } catch (error) {
  ...
  // 📊 Execute results via API — channel-aware
  async executeResults(chatId) {
    try {
      ...
      const body = channelConfig ? { channelId: channelConfig.channel_id } : {};
      const response = await axios.post(`${this.getBaseUrl()}/api/manual/results`, body, {
        headers: { 'Content-Type': 'application/json', 'x-bot-internal': 'true' },
        timeout: 60000
      });
...
const keyboard = [[{ text: '👤 Get Personal Coupons', url: 'https://t.me/Sportmsterbot?start=join_personal' }]];
  ...
  // 📰 Execute News - fetch and send latest news
  // 📰 Execute news — channel-aware
  async executeNews(chatId) {
    try {
      ...
      if (typeof result === 'string') {
        await telegram.sendNews(result, null, channelConfig);
      } else {
        await telegram.sendNews(result.content, result.originalItem, channelConfig);
      }

      const targetCh = channelConfig?.channel_id || telegram.channelId;
      await this.bot.sendMessage(chatId, `✅ News sent to <b>${targetCh}</b>!`, { parse_mode: 'HTML' });
    } catch (e) {
  ...
  async executeSendTargeted(chatId) {
    try {
      ...
      // Call the existing targeted coupons API
      const axios = require('axios');
      const baseUrl = process.env.PUBLIC_BASE_URL || 
                     (process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}` : 'https://idosegev23.vercel.app');
