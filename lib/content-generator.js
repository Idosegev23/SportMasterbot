...
class ContentGenerator {
  ...
  // ===== Aviator (Crash) Content =====
  // Short promotional tip for Aviator (max ~4 lines)
  async generateAviatorQuickTip(promoCode = 'SM100') {
    try {
      const prompt = `You are a concise Aviator (crash) coach for SportMaster Telegram.
Write a VERY SHORT, HIGH-CONVERTING English tip (max 5 lines) that feels premium and exciting.
Include:
• Hook line with ✈️ or ⚡
...
`;
      ...
      const response = await this.openai.chat.completions.create({
        model: 'gpt-4o-mini',
        messages: [
          { role: 'system', content: 'Professional Aviator coach. Output ONLY concise English. Keep it practical and promotional without guarantees.' },
      ...
      let content = this.cleanTelegramHTML(response.choices[0].message.content.trim());
      // Header
...
Rules:
- ENGLISH ONLY, Telegram HTML (<b>, <i>, <code>)
...
`;
      ...
      const response = await this.openai.chat.completions.create({
        model: 'gpt-4o-mini',
        messages: [
          { role: 'system', content: 'Professional Aviator planner. English only. Keep it short, clear, promotional, no guarantees.' },
          { role: 'user', content: prompt }
        ],
        max_tokens: 200,
        temperature: 0.6
  ...
  // Promotional Aviator message (2–3 sentences)
  async generateAviatorPromo(promoCode = 'SM100') {
    try {
      const prompt = `You are an ENGLISH marketing expert for SportMaster.
...
`;
      ...
      const response = await this.openai.chat.completions.create({
        model: 'gpt-4o-mini',
        messages: [
          { role: 'system', content: 'ENGLISH ONLY. Clean, concise marketing for Aviator. No guarantees.' },
          { role: 'user', content: prompt }
        ],
        max_tokens: 160,
        temperature: 0.8
...
`;
      ...
      // Score and rank results using same logic as match selection
      const scoredResults = recentResults.map(result => ({
        ...result,
        score: this.calculateResultScore(result)
      }))
      .sort((a, b) => b.score - a.score)
      .slice(0, 5); // Take only top 5 results

      // Simple format header
      let formattedResults = `🏆 <b>TOP RESULTS</b>\n\n`;
      
      // Add only top results
      scoredResults.forEach(result => {
        formattedResults += `⚽ <b>${result.homeTeam} ${result.homeScore}-${result.awayScore} ${result.awayTeam}</b>\n`;
      });
      
      // Add short AI commentary
      formattedResults += `\n🔥 ${content.split('.')[0]}.\n\n`;
      
      // Add footer
      formattedResults += `💎 Code: ${promoCode}`;
  ...
  // Calculate score for results (similar to match scoring)
  calculateResultScore(result) {
    ...
    if (bigTeams.some(team => (result.homeTeam || '').includes(team))) score += 25;
    if (bigTeams.some(team => (result.awayTeam || '').includes(team))) score += 25;
    
    // High-scoring games bonus
...
${liveMatches.map(m => {
  ...
  if (m.events && m.events.length > 0) {
    const recentEvents = m.events.slice(-2); // Last 2 events
    stats.push(`Recent: ${recentEvents.map(e => `${e.type} ${e.player || ''}`.trim()).join(', ')}`);
  }
  return `${m.homeTeam} vs ${m.awayTeam} — ${stats.join(' | ')} (${m.competition || 'Football'})`;
}).join('\n')}

Write a SHORT tactical live update in English:
...
`;
      ...
      const response = await this.openai.chat.completions.create({
        ...
        max_tokens: 120, // Shorter but meaningful commentary
        temperature: 0.8
      });

      let commentary = this.toTelegramHTML(response.choices[0].message.content.trim());

      // Compact format with integrated commentary
      ...
      liveMatches.forEach((m, index) => {
        ...
        // Only show recent IMPORTANT events (goals, red cards)
        if (m.events && m.events.length > 0) {
          const importantEvents = m.events.filter(e => e.type === 'Goal' || (e.type === 'Card' && e.detail === 'Red Card'));
          if (importantEvents.length > 0) {
            const recentImportant = importantEvents.slice(-1); // Just the latest important event
            formatted += ` — ${recentImportant.map(e => {
              const icon = e.type === 'Goal' ? '⚽' : '🟥';
              return `${icon} ${e.player || ''}`;
            }).join('')}`;
          }
        }
      ...
      // Always add tactical commentary if available - this gives context
