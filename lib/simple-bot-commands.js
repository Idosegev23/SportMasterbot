...
⚙️ <b>System</b> — Status & analytics`;
  ...
  // 📰 Execute news DIRECTLY — channel-aware, fast (no slow image gen)
  async executeNews(chatId) {
    try {
      ...
      if (typeof result === 'string') {
        ...
        newsText = result.content;
      } else if (Array.isArray(result)) {
        newsText = result.join('\n\n');
      }
      ...
      // Sanitize for HTML: convert Markdown bold/italic to HTML, escape special chars
      newsText = newsText
      ...
      if (newsText.length > 4000) newsText = newsText.substring(0, 3997) + '...';

      const keyboard = await telegram.buildKeyboard(channelConfig, 'news');

      // Send directly — skip image gen for speed (webhook has 60s limit)
      const msg = await telegram.bot.sendMessage(targetChannel, newsText, {
        parse_mode: 'HTML',
        disable_web_page_preview: true,
        reply_markup: { inline_keyboard: keyboard }
      });
      try { await telegram.logPostToSupabase('news', newsText, msg.message_id, {}, channelConfig); } catch (_) {}
