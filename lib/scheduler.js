...
class GizeBetsScheduler {
  ...
  async executeManualResults() {
    try {
      ...
      // Get ALL yesterday results first
      const allResults = await this.footballAPI.getYesterdayResults();
      console.log(`📊 Found ${allResults.length} total results from yesterday`);
      
      // Filter to only matches we predicted
      let filteredResults = allResults;
      try {
        const { supabase } = require('./supabase');
        if (supabase) {
          console.log('🔍 Filtering results to only matches we predicted...');
          
          // Get matches from last 48 hours that we sent predictions for
          ...
          if (!error && posts && posts.length > 0) {
            ...
            if (predictedPairs.size > 0) {
              console.log(`📋 Found ${predictedPairs.size} predicted matches`);
              filteredResults = allResults.filter(result => {
                const homeTeam = result.homeTeam?.name || result.homeTeam || '';
                const awayTeam = result.awayTeam?.name || result.awayTeam || '';
                return predictedPairs.has(`${homeTeam.toLowerCase()}__${awayTeam.toLowerCase()}`);
              });
              console.log(`🎯 Filtered to ${filteredResults.length} results we predicted`);
            }
          }
        }
      } catch (e) {
        console.log('⚠️ Error filtering results from Supabase:', e.message);
        // Fallback: keep top 5 results from premier leagues only
        filteredResults = allResults.slice(0, 5);
        console.log(`🔄 Fallback: showing top ${filteredResults.length} results`);
      }
      
      if (filteredResults.length === 0) {
        throw new Error('No relevant results available for yesterday');
      }
      
      const content = await this.contentGenerator.generateDailyResults(filteredResults);
      const result = await this.telegram.sendResults(content, filteredResults);
  ...
  async executeManualHype() {
    try {
      ...
      // Extract match metadata for live tracking
      const matchMetadata = {
      ...
      await this.telegram.logPostToSupabase('today_hype', content, msg.message_id, matchMetadata);
