...
👤 <b>Admin:</b> ${new Date().toLocaleString('en-US', { timeZone: 'Africa/Addis_Ababa' })} (ET)`;
    ...
    const keyboard = [
      ...
      [
        ...
        { text: '📰 Send News', callback_data: 'cmd_news' }
      ],
      [
  ...
  async setupWebhook() {
    try {
      ...
      // Always prefer PUBLIC_BASE_URL, then VERCEL_URL, then hardcoded fallback
...
const keyboard = [[{ text: '👤 Get Personal Coupons', url: 'https://t.me/Sportmsterbot?start=join_personal' }]];
  ...
  async executeNews(chatId) {
    try {
      ...
      // Simple news headlines - you can enhance this with actual RSS feeds
      const headlines = [
        "🏆 Champions League: Latest match results and upcoming fixtures",
        "⚽ Premier League: Transfer news and team updates", 
        "🏅 World Cup Qualifiers: Key matches this week",
        "📈 Football Analytics: Top performers this season"
      ];
      
      await telegram.sendNews(headlines);
      await this.bot.sendMessage(chatId, '✅ News sent to channel successfully!');
      
    } catch (e) {
      console.error('❌ executeNews error:', e);
      await this.bot.sendMessage(chatId, '❌ Failed to send news: ' + (e.message || e));
    }
  }
